...
namespace Application.Features.DepenseFeatures.Commands
{
    public class CreateDepenseCommandValidator : AbstractValidator<CreateDepenseCommand>
    {
        ...
        public CreateDepenseCommandValidator(IApplicationDbContext context)
        {
            RuleSet("CreateDepense", () =>
            {
        ...
        private bool BeDifferentAmount(CreateDepenseCommand command, double montant)
        {
           return ! context.Depenses.Any(d => d.UserId == command.UserId && d.Montant == montant && d.Date == command.Date);
         
        }

        private bool DateExist(CreateDepenseCommand command)
        {
            return context.Depenses.Any(d => d.UserId == command.UserId && d.Date == command.Date);
        }      

        private Task<bool> BeSameDeviseAsUser(CreateDepenseCommand command, int devise, CancellationToken arg2)
        {
